...
void SimplInvIndexQA1<dist_t>::Search(KNNQuery<dist_t>* query, IdType) const {
  ...
  pSpace_->extractBM25QueryVector(query_vect, o);
...
template <typename dist_t>
void SimplInvIndexQA1<dist_t>::CreateIndex(const AnyParams& IndexParams) {
  AnyParamManager pmgr(IndexParams);
  CreateIndex(pmgr);
}

template <typename dist_t>
void SimplInvIndexQA1<dist_t>::CreateIndex(AnyParamManager& ParamManager) {
  ...
  for (const Object* o : data_) {
    tmp_vect.clear();
    pSpace_->extractBM25DocVector(tmp_vect, o);
  ...
  for (size_t did = 0; did < data_.size(); ++did) {
    tmp_vect.clear();
    pSpace_->extractBM25DocVector(tmp_vect, data_[did]);
...
template <typename dist_t>
SimplInvIndexQA1<dist_t>::~SimplInvIndexQA1() {
// nothing here yet
}

template <typename dist_t>
void 
SimplInvIndexQA1<dist_t>::SetQueryTimeParams(const AnyParams& QueryTimeParams) {
...
template class SimplInvIndexQA1<float>;
