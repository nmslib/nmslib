...
\subsubsection{MI-File.}
Another approach, called MI-File, relies on the inverted index over permutations \cite{amato2008approximate}.
...
For each data point, we select \ttt{numPivotIndex} $\ll$ \ttt{numPivot} pivots 
...
\todonoteinline{Parameters need to be synched with the VLDB paper, we have a lot of new ones,
but we don't want to document all of them here.}
