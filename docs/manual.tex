...
Specifically, the maximum number of neighbors in all layers but the ground layer is \ttt{M} (an index-time parameter).
The maximum number of neighbors for the ground layer is \ttt{M0} (an index-time parameter). 
...
\todonoteinline{Modify default values for SearchMethod and query time parameters}
\todonoteinline{Add examples and update parameter tables}
\todonoteinline{Saving the index, but only for L2 and the cosine-similarity}
