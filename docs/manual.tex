...
For high recall values ($>99$\%) the good value is 1. Otherwise, the value of 0 is a good choice (which
also leads to shorter indexing times).
...
Finally, we note that for very high recall values ($>99$\%) setting \ttt{delaunay\_type} to 1 improves retrieval times.
This comes at the expense of longer indexing times. Therefore, for lower recall values, we recommend using \ttt{delaunay\_type}=0.
