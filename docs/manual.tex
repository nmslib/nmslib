...
We do not have a space with integer-valued distance function yet, but we
...
two expensive operations are computing ground truth answers
and indexing.
Currently, we recompute ground truth data every time we benchmark, which may take long time.
However, substantial time savings can be achieved by benchmarking several methods
using a single invocation of the binary \ttt{experiment}. In this case,
ground truth data is computed once for all tested methods.
...
In future versions, we plan to address this issue and to compute ground truth data before any experimentation begins 
...
or the Visual Studio (Clang seems to be equally efficient).
...
\item Miscellaneous methods \S~\ref{SectionMiscMeth}, which also include a \knn graph implementation \S~\ref{SectionKNNGraph};
\end{itemize}
...
when using $\alpha$ possibly smaller than one \cite{Boytsov_and_Bilegsaikhan:nips2013}.
...
The mnemonic code of this (faster) modification is \ttt{perm\_incsort}.


The following is an example of testing the incremental-sort permutation method with the benchmarking utility \ttt{experiment}:
{
\footnotesize
\begin{verbatim}
release/experiment \
...
A method can have query time parameters, which can be changed
...
to the file
\href{\replocfile similarity_search/test/test_integr.cc#L65}{test\_integr}. 
...
If you are using the script  \href{\replocfile sample_scripts/sample_scripts/genplot.py}{genplot.py} (see \S~\ref{SectionGenPlot}) to plot performances graphs, you will need to modify it as well.
The function \ttt{methodNameAndStyle} in this script defines a mapping from the method name
to plot marker. Currently, we provide markers for all the implemented methods. 
