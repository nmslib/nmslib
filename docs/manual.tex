...
We would re-iterate that one needs 64-bit Boost binaries compiled with the same version of the Visual Studio as the \nonmetrlib{} binaries.  
...
\end{verbatim}
Ground truth data is stored in two files. One is a textual meta file
...
This check, which proved to be quite useful, fail in two cases:
\begin{itemize}
\item The user creates a gold standard cache. Then, he modifies a distance function of the space
and runs the tests again;
...
\todonoteinline{Also the number of test threads and creation of the gold standard cache!}
...
The plotting script is \href{\replocfile sample_scripts/genplot.py}{genplot.py}.
