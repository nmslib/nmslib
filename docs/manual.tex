...
The Hierarchical Navigable Small World Graph (HNSW) \cite{Malkov2016} is an improved variant of the SW-graph.
...
HNSW can be seen as a multi-layer and a multi-resolution version of the SW-graph. 
...
The new point is added to all layers from zero to $m$ using a search-based algorithm very similar to that of the 
basic SW-graph. 
The search starts from the maximum-level layer and proceeds to lower layers by searching one layer at a time.
The closest point found at the layer $h+1$ is used as a starting point for the search carried out at the layer $h$. 
The search procedure is a previously described greedy search algorithm whose quality is controlled 
by the parameter \ttt{efConstruction}.
Note though that for layers higher than $m$, we always use \ttt{efConstruction}=1.

An outcome of a search in a layer is a set of data points that are close to the new point.
These data points become neighbors of the new point in the layer's graph.
...
The closest point found at the layer $h+1$ is used as a starting point for the search carried out at the layer $h$. 
For all layers but the ground layer, we always use \ttt{efSearch}=1.
When we reach the ground layer, we carry out the regular search algorithm whose quality is controlled by the parameter \ttt{efSearch}.
Note that the algorithm does not carry out multiple sub-searches starting from different random data points.

For $L_2$ and the cosine similarity, HNSW has optimized implementations. To enable them,
one has to use a query-time parameter \ttt{searchMethod}. 
Setting \ttt{searchMethod} to 3 and 4 enables optimized implementations for $L_2$ and the cosine similarity, respectively.
The default value of \ttt{searchMethod} is zero, which corresponds to a generic algorithm.
...
HNSW is capable of saving an index for optimized implementations, i.e., when \ttt{SearchMethod} is 3 or 4. Here
is an example for the optimized $L_2$ implementation:
{
\footnotesize
\begin{verbatim}
release/experiment \
...
\cmidrule(l){1-2} 
\ttt{mult}                & A scaling coefficient to determine the depth of a layer (see the paper by Malkov and Yashunin \cite{malkov2014} for details). A default value seems to be good enought. \\
\ttt{searchMethod}        & A type of the search algorithm: 0 means a generic implementation,
                            3 means an $L_2$-optimized version, and 4 means a version optimized for the cosine similarity.      \\
\ttt{maxM}                & The maximum number of neighbors in all layers but the ground layer. \\
\ttt{maxM0}               & The maximum number of neighbors in the \emph{ground} layer (default values seems to be good enough). \\
